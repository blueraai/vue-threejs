...
description: Renderer-specific behaviors to know when using Vue Three Fiber
---

Vue Three Fiber has a few renderer-specific behaviors that come from Vue's renderer architecture. This page documents each one and what to do about it.

## Refs

Template refs resolve to a proxy-backed handle instead of a raw `THREE.Object3D` instance.
...
Vue's `<Suspense>` component works with async `setup()` functions and `useLoader`, but its re-entrance behavior matters when a resolved branch is replaced by a new pending branch.
...
> [!NOTE]
> For new code, prefer the Vue-native render lifecycle APIs — [`useRenderCommit`](/API/hooks#userendercommit) (wait for Vue flush + scene render) and [`useNextFrame`](/API/hooks#usenextframe) (wait for one rendered frame). `flushSync` is kept as a lower-level compatibility utility for imperative scene coordination.
