...
## Future: @vue-three/extras

Higher-level components like `SceneTransition` and `ResourceBoundary` are planned for the `@vue-three/extras` package. These will formalize the patterns shown in this tutorial into reusable components with explicit transition strategy props (`keep-previous`, `replace`, `crossfade`), built-in progress tracking, and optional enter/leave animations for scene content.
